...
Player.prototype.initialise = function()
{
	...
	if (this.debug) {
		...
		html += "<tr><th>Menu Transcode</td><th>" + localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "enableTranscoding") + "</td></tr>";
		html += "<tr><th>Direct Play</td><th>" + self.directPlay + "</td></tr>";
		html += "<tr><th>Bit Depth</td><th>" + self.bitdepth + "</td></tr>";
		html += "</table>";
		$("#debug").html(html);				
		$("#debug").show();
				...
				switch ($(self.cache).find("Video:first").attr("type")) {
					case "episode":	
						...
						self.plex.getMediaMetadata(parentKey + "/children", function(xml) {
							...
							if (nextKey) {
								$("#message").html("Next Episode in<br/>3");
								$("#message").show().delay(1000).hide();
								$("#message").html("Next Episode in<br/>2");
								$("#message").show().delay(1000).hide();
								$("#message").html("Next Episode in<br/>1");
								$("#message").show().delay(1000).fadeOut(1000);
...
Player.prototype.openMedia = function(key)
{
	...
	this.plex.getMediaMetadata(this.key, function(xml) {
		...
		self.hideLoader();
		$("#message").html("01").show().fadeOut(3000);
		self.mediaUrl = $(xml).find("Part:first").attr("key");
		$("#message").html("02").show().fadOout(3000);
		self.viewOffset = $(xml).find("Video:first").attr("viewOffset");
		$("#message").html("03").show().fadeOut(3000);
		self.duration = $(xml).find("Video:first").attr("duration"); 
		$("#message").html("04").show().fadeOut(3000);
		self.bitdepth = $(xml).find("Media:first Part:first Stream[streamType='1']").attr("bitDepth"); 
		$("#message").html("05").show().fadeOut(3000);
		self.reframes = $(xml).find("Media:first Part:first Stream[streamType='1']").attr("refFrames");
		$("#message").html("06").show().fadeOut(3000);;
		self.subformat = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1']").attr("format"); 
		$("#message").html("07").show().fadeOut(3000);
		self.subcodec = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1']").attr("codec"); 
		$("#message").html("08").show().fadeOut(3000);
		//self.subformat = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1'][format='srt']"); 
		//self.subcodec = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1'][codec='srt']"); 
		self.subindex = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1']").attr("index");
		$("#message").html("09").show().fadeOut(3000);
		//self.subselected = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1']").attr("selected");
		self.subselected = $(xml).find("Media:first Part:first Stream[streamType='3'][selected='1']").attr("selected");
		$("#message").html("10").show().fadeOut(3000);
		$("#message").html(self.bitdepth + self.reframes + self.subindex + self.subformat + self.subcodec + self.subselected).show().fadeOut(3000);
		self.transcode = ((self.bitdepth > 8) || (self.reframes == "16") || (self.subindex > 0 && self.subformat != "srt") || (self.subformat != "srt" && self.subcodec != "srt" && self.subselected > 0)) ? true : false;
		$("#message").html("11").show().fadeOut(3000);
		//self.directPlay = ((self.bitdepth > 8) || (self.subindex) || (self.subformat != "srt" && self.subselected == "1" )) ? false : self.directPlay;
		//self.partKey = $(xml).find("Media:first Part:first").attr("id");
		
		//To Do: Check for multiple media streams
		/*setTimeout(function() {
			if ((self.bitdepth > 8) || (self.subindex) || (self.subformat != "srt" &&  self.subselected == "1" )) {
				self.directPlay = false;
			} else {
				self.directPlay = (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "enableTranscoding") == "1") ? false : true;
				//self.directPlay = true;
			}
		},1000);
		$("#message").html("Bits: " + self.bitdepth + "<br/>Index: " + self.subindex.length + "<br/>SubFormat: " + self.subformat + "<br/>Selected: " + self.subselected.length + "<br/Transcode: " + self.transcode + "<br/>DPlay:" + self.directPlay);
		$("#message").show().delay(8000).fadeOut(3000);*/
		if (self.directPlay && self.transcode == false) {
			$("#message").html("12a").show().fadeOut(3000);
			//Direct play
			...
			$("#message").html("12b").show().fadeOut(3000);
			self.media.data = self.plex.getHlsTranscodeUrl(self.key);
			//self.transcode = true;
		}
		self.hideLoader();
		$("#message").html("13").show().fadeOut(3000);
		if ($.querystring().autoplay == "true") {
			if (self.viewOffset) {
				...
				if (self.transcode) {
					$("#subtitles").hide();
					//self.plex.setSubtitleStream($(xml).find("Media:first Part:first").attr("id"));
...
Player.prototype.resumeDialog = function(ms)
{
	...
	$("#resume").click(function() {
		...
		if (self.directPlay) {
			...
			self.resume = true;
			//self.plex.getTimeline(self.mediaKey, "playing", ms, self.duration, encodeURIComponent($(this.cache).find("Video:first").attr("guid")));
		}
...
Player.prototype.play = function(speed)
{
	...
	this.timer = setInterval(function() {
		...
		if (self.media.playPosition <= self.media.playTime && self.media.playPosition > 0) {
			self.plex.getTimeline(self.mediaKey, "playing", self.media.playPosition, self.media.playTime, encodeURIComponent($(self.cache).find("Video:first").attr("guid")));
			//self.plex.reportProgress(self.mediaKey, "playing", self.media.playPosition);
		}
		self.progressCount++;
		
		if (self.progressCount >= 10 && self.media.playPosition+1 <= self.media.playTime) {
...
Player.prototype.rewind = function()
{
	...
	this.media.seek(pos);
	var ppos = (self.media.playPosition/self.media.playTime)*100; 
	self.progessbar.progress(ppos);
 ...
 */	
	...
	this.media.seek(pos);
	// clearInterval(this.timer);
	// this.media.play(self.speed);
	var ppos = (self.media.playPosition/self.media.playTime)*100; 
	self.progessbar.progress(ppos);
	
	$("#message").html("<i class=\"glyphicon xlarge forward\"></i>");
	// $("#message").html("<i class=\"glyphicon xlarge forward\"></i><br/>"+self.speed+"x");
	$("#message").show();
	$("#message").fadeOut(3000);	
};
...
{ 
        ...
        if (code == 'por' || code == 'pob' || code == '') { return 'pt'; }; 
